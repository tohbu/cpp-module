...
private:
	...
	template < typename T >
	void sortContainer(T &container, int block_size);
	template < typename T >
	...
	template < typename T >
	void make_main_chain(T &container, int block_size);
...
// template < typename T >
// void PmergeMe< T >::printContainer(const T &container, int block_size)
// {
...
// template < typename T >
// PmergeMe< T >::PmergeMe(int argc, char *argv[], T container, std::string container_type) : _nbr_of_comps(0), _container(container)
// {
...
// 	printResults(start, end, container_type, this->_container.size());
