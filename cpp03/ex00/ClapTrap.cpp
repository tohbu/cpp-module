...
{
	pMsg("Default constructor called");
}
ClapTrap::ClapTrap(std::string name) : _name(name), _HitPoint(10), _EnergyPoint(10), _AttackDamage(0)
{
	pMsg("Name constructor called");
}

ClapTrap::ClapTrap(const ClapTrap&c) :_name(c._name), _HitPoint(c._HitPoint), _EnergyPoint(c._EnergyPoint), _AttackDamage(c._AttackDamage)
{
...
void ClapTrap:: takeDamage(unsigned int amount)
{
	...
	_HitPoint -=amount;
	if(_HitPoint <= 0)
		std::cout<<"ClapTrap " <<"<" <<_name << "> " << "game over" <<std::endl;
}
